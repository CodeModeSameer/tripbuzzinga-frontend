...
import styles from "./CategoryItineraryPage.module.css";
...
export default function CategoryItineraryPage({ slug, itineraryId }) {
  ...
  if (!itinerary) {
    return (
      <div className={styles.page} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '80vh' }}>
        ...
        <Link href={`/categories/${slug}`} className={styles.backBtn} style={{ background: '#3b82f6', color: 'white', padding: '12px 24px', borderRadius: '8px', textDecoration: 'none' }}>
  ...
  return (
    <>
      ...
      {/* Back Navigation */}
      <Link href={`/categories/${slug}`} className={styles.backBtn}>
        <ArrowLeft size={18} />
